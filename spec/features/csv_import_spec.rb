...
RSpec.describe 'csv import' do
  it 'with valid csv' do
    ...
    expect(Event.find_by(url: 'http://www.swingpatrol.co.uk/boogaloo-bounce/').swing_dates.count).to eq 0
    expect(Event.find_by(url: 'http://www.swingpatrol.co.uk/book-club-blues/').swing_dates.count).to eq 0

    click_on 'Save'

    expect(page).to have_content 'New Venue' # i.e. on the admin homepage

    boogaloo_bounce = Event.find_by(url: 'http://www.swingpatrol.co.uk/boogaloo-bounce/')
    expect(boogaloo_bounce.swing_dates.count).to eq 24

    book_club_blues = Event.find_by(url: 'http://www.swingpatrol.co.uk/book-club-blues/')
