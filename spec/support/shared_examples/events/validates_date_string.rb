...
RSpec.shared_examples "validates date string" do |attribute, model_name, options = {}|
  ...
  it "is invalid with a non-date string" do
    ...
    model.valid?
    expect(model.errors.messages[attribute]).to include('contained some invalid dates: "foo"')
  end
  ...
  it "is invalid with several non-date strings" do
    ...
    model.valid?
    expect(model.errors.messages[attribute]).to include('contained some invalid dates: "foo", "30//2012", "06/30/2012"')
  end
  ...
  it "is invalid with a string with a mix of valid and invalid date strings" do
    ...
    model.valid?
    expect(model.errors.messages[attribute]).to include('contained some invalid dates: "foo", "06/30/2012"')
  end
  ...
  it "is invalid if it's missing commas" do
    ...
    model.valid?
    expect(model.errors.messages[attribute]).to include('contained some invalid dates: "30/12/2012 30/06/2012"')
  end

  it "is invalid if has extra stuff in it" do
    model = build(model_name, attribute => "30/12/2012foo")
    model.valid?
    expect(model.errors.messages[attribute]).to include('contained some invalid dates: "30/12/2012foo"')
  end
  ...
  if options[:allow_past]
    ...
    it "is invalid if dates are earlier than the start of SOLDN" do
      ...
      model.valid?
      expect(model.errors.messages[attribute]).to include("contained some dates in the distant past: 15/08/2010")
    end
  else
    it "is invalid if dates are in the past" do
      model = build(model_name, attribute => "31/05/2012")
      model.valid?
      expect(model.errors.messages[attribute]).to include("contained some dates in the past: 31/05/2012")
    end
  end
  ...
  it "is invalid if dates are too far in the future" do
    ...
    model.valid?
    expect(model.errors.messages[attribute]).to include("contained some dates unreasonably far in the future: 02/06/2014")
  end

  it "is invalid with duplicate dates" do
    model = build(model_name, attribute => "12/07/2012,12/07/2012")
    model.valid?
    expect(model.errors.messages[attribute]).to include("contained some dates more than once: 12/07/2012")
  end

  it "only reports duplicate future dates once as future" do
    model = build(model_name, attribute => "01/02/2020,01/02/2020")
    model.valid?
    expect(model.errors.messages[attribute]).to include(
      "contained some dates unreasonably far in the future: 01/02/2020",
  ...
  it "reports duplicated non-dates twice" do
    ...
    model.valid?
    expect(model.errors.messages[attribute]).to include('contained some invalid dates: "30/02/2012", "30/02/2012"')
  end
end
