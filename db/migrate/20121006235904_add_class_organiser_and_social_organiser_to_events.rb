class AddClassOrganiserAndSocialOrganiserToEvents < ActiveRecord::Migration
  def up
    ...
    Event.all.each do |event| 
      event.update_attributes(class_organiser_id: event.organiser_id) if event.has_class || event.has_taster
      event.update_attributes(social_organiser_id: event.organiser_id) if event.has_social
    end
  ...
  def down
    ...
    Event.all.each do |event| 
      org_id =  if event.class_organiser_id
                  event.class_organiser_id
                else
                  event.social_organiser_id
                end
      event.update_attributes(organiser_id: org_id)
    end
